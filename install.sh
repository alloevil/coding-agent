...
echo ""
echo "✅ Installed. Just run it — the first launch walks you through setup"
echo "   (provider, API key, model); no environment variables needed."
echo ""
if [ -n "$TUI_BIN" ]; then
  ...
  echo ""
  echo "   (if the TUI can't find python, prefix: CODING_AGENT_PYTHON=$VENV/bin/python)"
else
  echo "   $VENV/bin/coding-agent          # plain CLI (guided setup on first run)"
  echo "   $VENV/bin/coding-agent --tui    # built-in rich TUI"
  echo "   (install Rust — https://rustup.rs — for the full-screen TUI)"
fi
echo ""
echo "   Advanced: skip the wizard by exporting MODEL_API_KEY / MODEL_BASE_URL /"
echo "   MODEL_PRIMARY (or OPENAI_API_KEY). Also: make run / make tui-run."
